...
URL=https://sourceforge.net/projects/android-rom-ota/files/LineageOS-OMS/"$device"/"$FILE_NAME"-OMS.zip/download
...
sed -i '3c \    {' $JSON
sed -i '4c \      "filename": "'$FILE_NAME'-OMS",' $JSON
...
OTAXML=$DIR_1/android_ota/lineage-oms/$device.xml

sed -i '5c <Filename>'$FILE_NAME_2''$device'-OMS</Filename>' $OTAXML
sed -i '6c <RomUrl id="rom" title="Download ROM">https://sourceforge.net/projects/android-rom-ota/files/LineageOS-OMS/'$device'/'$FILE_NAME'-OMS.zip/download</RomUrl>' $OTAXML

cd /home/audahadi/lineage-n/android_ota
git add -A && git commit -am "$FILE_NAME-OMS"
